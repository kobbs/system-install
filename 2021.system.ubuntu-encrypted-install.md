...
sudo mkfs.ext4 -L USBBOOT /dev/sd(x)3
sudo mkswap /dev/sdf4
sudo cryptsetup -c aes-xts-plain64 -y -s 256 luksFormat /dev/sd(x)5
sudo cryptsetup luksOpen /dev/sdf5 nvme2000root
...
echo "usbroot UUID=${UUIDCRYPT} none luks" >> /etc/crypttab
```

```sh
blkid -s UUID -o value /dev/sdf2  
8839-820B
blkid -s UUID -o value /dev/sdf3
...
/dev/mapper/usbroot / ext4 noatime,defaults 0 1
```
...
xorg gdm3 i3 i3lock i3status bash-completion htop ca-certificates network-manager network-manager-gnome network-manager-openvpn-gnome pulseaudio alsa-utils pavucontrol terminator telnet git numlockx apt-transport-https dosfstools openvpn curl gnupg2 amd64-microcode software-properties-common lxappearance scrot fakeroot build-essential dkms
